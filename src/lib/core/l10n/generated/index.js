/**
 * DO NOT CHANGE this file
 * It has been generated by vite translator plugin 
 */

export const LocalizationBundle = {
  "en": {
    "core": {},
    "idCardScanner": {
      "guid": {
        "title": "ขั้นตอนกา  รสแกนบัตรประชาชน",
        "desc": "<ul>\n    <br>\n      <li>  •  หามุมหารถ่ายที่มีแสงสว่างเพียงพอ โดยที่จะเห็นข้อมูลบัตร </li>\n      <br>\n      <li>  • โปรดระวังนิ้วหรือสิ่งของที่อาจบัง ข้อมูลบนบัตรประชาชนของคุณ </li><br>\n      <li>  • แสงสะท้อนของบัตรอาจะทำให้การ ตรวจสอบข้อมูลทำได้ยากขึ้น</li><br>\n  </ul>",
        "footerBtn": "Next"
      },
      "scan": {
        "title": "กรุณาวางบัตรให้อยู่ในกรอบที่กำหนด ให้เห็นตัวอักษร และข้อมูลบนหน้าบัตรชัดเจน",
        "preview": {
          "title": "สแกนบัตรประชาชน",
          "footerPrimaryBtn": "Submit",
          "footerSecondaryBtn": "ถ่ายใหม่อีกครั้ง"
        }
      },
      "form": {
        "title": "ขั้นตอนการสแกนบัตรประชาชน",
        "inputFieldCitizen": {
          "label": "เลขบัตรประชาชน"
        },
        "inputFieldName": {
          "label": "ชื่อ"
        },
        "inputFieldSurname": {
          "label": "นามสกุล"
        },
        "footerBtn": "Next"
      }
    },
    "preScreen": {
      "footerBtn": "Next",
      "symptomSelection": {
        "title": "กรุณาระบุอาการที่ต้องการรับการปรึกษา",
        "subTitle": "อาการที่สามารถใช้สิทธิการรักษาพยาบาลตามเงื่อนไข การให้บริการจากสปสช.",
        "additionalSymptoms": {
          "title": "กรอกอาการเพิ่มเติม",
          "placeholder": "โปรดระบุอาการเบื้องต้น"
        },
        "warnDialogServiceNotAvailable": {
          "title": "ขออภัย ไม่สามารถให้บริการได้",
          "desc": "เนื่องจากสิทธิการรักษาพยาบาลอยู่นอกเงื่อนไขการให้บริการจากสปสช. หรือมีอาการเจ็บป่วยที่ควรรับการรักษา ณ สถานพยาบาล",
          "btn": "รับทราบ"
        },
        "warnDialogRequiredField": {
          "title": "เลือกหน่อยค่ะ้",
          "desc": "เลือกให้หนูหน่อยค่ะ",
          "btn": "เข้าใจ?"
        }
      },
      "userInfo": {
        "height": {
          "title": "ส่วนสูง",
          "unit": "ซม."
        },
        "weight": {
          "title": "น้ำหนัก",
          "unit": "กก."
        },
        "disease": {
          "title": "โรคประจำตัว",
          "value": {
            "yes": "มี",
            "no": "ไม่มี"
          },
          "detail": {
            "placeholder": "โปรดระบุโรคประจำตัว"
          }
        },
        "drugAllergy": {
          "title": "ประวัติการแพ้ยา",
          "value": {
            "yes": "มี",
            "no": "ไม่มี"
          },
          "detail": {
            "placeholder": "โปรดระบุประวัติการแพ้ยา"
          }
        },
        "selectionBirthDate": {
          "title": "",
          "year": {
            "placeholder": "ปี"
          },
          "month": {
            "placeholder": "เดือน"
          },
          "date": {
            "placeholder": "วัน"
          }
        }
      }
    },
    "selfie": {
      "guid": {
        "title": "การถ่ายรูปคู่บัตรประชาชน",
        "desc": " <ul>\n        <li>  • บัตรต้องไม่บังใบหน้าส่วนใดส่วนหนึ่ง </li>\n      <br>\n      <li>  • ถอดแว่น ถอดแมสก่อนถ่าย </li><br>\n      <li>  • รูปต้องชัดเจนให้เห็นตัวอักษรบนบัตร</li><br>\n    </ul>",
        "footerBtn": "Next"
      },
      "scan": {
        "title": "กรุณาถ่ายรูปคู่ท่านและบัตรประชาชน ให้เห็นตัวอักษร และข้อมูลบนหน้าบัตรชัดเจน",
        "preview": {
          "title": "ยืนยันรูปคู่บัตรประชาชน",
          "footerPrimaryBtn": "Submit",
          "footerSecondaryBtn": "ถ่ายใหม่อีกครั้ง"
        }
      }
    },
    "reservePrivilege": {
      "checking": {
        "title": "ระบบกำลังทำการ ตรวจสอบสิทธิ์ของท่าน",
        "subtitle": "ทางระบบจะทำการจะส่งแจ้งเตือนไปยัง <br> ลูกค้าเมื่อได้ทำการเช็คสิทธิ์เรียบร้อย"
      },
      "confirmed": {
        "title": "สิทธิของท่านได้รับการ ยืนยันแล้ว!",
        "subtitle": "ระบบกำลังนำทางท่านไปพบคุณหมอ <br> ท่านสามารถสอบถามและปรึกษา อาการ ต่างๆ ของท่านได้เลย"
      },
      "rejected": {
        "title": "ขออภัย!สิทธิของท่านไม่ตรงตาม เงื่อนไขที่กำหนด",
        "subtitle": "กรุณาตรวจสอบสิทธิของท่านอีกครั้ง"
      }
    },
    "queuing": {
      "queueConfirmation": {
        "desc": "สามารถสอบถามและปรึกษา <br> อาการต่างๆของท่านได้เลย",
        "btnNavigateToMeet": "กดเข้าห้องเพื่อปรึกษาแพทย์"
      },
      "waitingMeetDoctor": {
        "title": "กรุณารอสักครู่ <br> คุณหมอกำลังเข้ามาให้<br> บริการท่านในอีกสักครู่ค่ะ",
        "subtitle": "ระบบกำลังนำทางท่านไปสู่การพบคุณหมอ <br> ท่านสามารถสอบถามและปรึกษาอาการ<br> ต่างๆ ของท่านได้เลย"
      }
    },
    "navigateBack": {
      "navigateBackNhso": {
        "title": "ขอบคุณสำหรับ\nการใช้บริการกับเรา",
        "desc": "ท่านสามารถกลับมาใช้สิทธิ์สปสช.\nผ่านหน้าหลักตามรูป",
        "btn": "กลับสู่หน้าหลัก"
      }
    },
    "welcomeNhso": {
      "welcomeNhso": {
        "title": "ยินดีต้อนรับสู่การใช้\nบริการสิทธิ์สปสช",
        "desc": "ระบบกำลังตรวจสอบสถานะ\nของท่านรุณารอสักครู่",
        "btn": "กลับสู่หน้าหลัก"
      }
    }
  },
  "th": {
    "core": {},
    "idCardScanner": {
      "guid": {
        "title": "ขั้นตอนการสแกนบัตรประชาชน",
        "desc": "<ul>\n    <br>\n      <li>  •  หามุมหารถ่ายที่มีแสงสว่างเพียงพอ โดยที่จะเห็นข้อมูลบัตร </li>\n      <br>\n      <li>  • โปรดระวังนิ้วหรือสิ่งของที่อาจบัง ข้อมูลบนบัตรประชาชนของคุณ </li><br>\n      <li>  • แสงสะท้อนของบัตรอาจะทำให้การ ตรวจสอบข้อมูลทำได้ยากขึ้น</li><br>\n  </ul>",
        "footerBtn": "ถัดไป"
      },
      "scan": {
        "title": "กรุณาวางบัตรให้อยู่ในกรอบที่กำหนด ให้เห็นตัวอักษร และข้อมูลบนหน้าบัตรชัดเจน",
        "preview": {
          "title": "สแกนบัตรประชาชน",
          "footerPrimaryBtn": "ยืนยัน",
          "footerSecondaryBtn": "ถ่ายใหม่อีกครั้ง"
        }
      },
      "form": {
        "title": "ขั้นตอนการสแกนบัตรประชาชน",
        "inputFieldCitizen": {
          "label": "เลขบัตรประชาชน"
        },
        "inputFieldName": {
          "label": "ชื่อ"
        },
        "inputFieldSurname": {
          "label": "นามสกุล"
        },
        "footerBtn": "ถัดไป"
      }
    },
    "preScreen": {
      "footerBtn": "ถัดไป",
      "symptomSelection": {
        "title": "กรุณาระบุอาการที่ต้องการรับการปรึกษา",
        "subTitle": "อาการที่สามารถใช้สิทธิการรักษาพยาบาลตามเงื่อนไข การให้บริการจากสปสช.",
        "additionalSymptoms": {
          "title": "กรอกอาการเพิ่มเติม",
          "placeholder": "โปรดระบุอาการเบื้องต้น"
        },
        "warnDialogServiceNotAvailable": {
          "title": "ขออภัย ไม่สามารถให้บริการได้",
          "desc": "เนื่องจากสิทธิการรักษาพยาบาลอยู่นอกเงื่อนไขการให้บริการจากสปสช. หรือมีอาการเจ็บป่วยที่ควรรับการรักษา ณ สถานพยาบาล",
          "btn": "รับทราบ"
        },
        "warnDialogRequiredField": {
          "title": "เลือกหน่อยค่ะ้",
          "desc": "เลือกให้หนูหน่อยค่ะ",
          "btn": "เข้าใจ?"
        }
      },
      "userInfo": {
        "height": {
          "title": "ส่วนสูง",
          "unit": "ซม."
        },
        "weight": {
          "title": "น้ำหนัก",
          "unit": "กก."
        },
        "disease": {
          "title": "โรคประจำตัว",
          "value": {
            "yes": "มี",
            "no": "ไม่มี"
          },
          "detail": {
            "placeholder": "โปรดระบุโรคประจำตัว"
          }
        },
        "drugAllergy": {
          "title": "ประวัติการแพ้ยา",
          "value": {
            "yes": "มี",
            "no": "ไม่มี"
          },
          "detail": {
            "placeholder": "โปรดระบุประวัติการแพ้ยา"
          }
        },
        "selectionBirthDate": {
          "title": "",
          "year": {
            "placeholder": "ปี"
          },
          "month": {
            "placeholder": "เดือน"
          },
          "date": {
            "placeholder": "วัน"
          }
        }
      }
    },
    "selfie": {
      "guid": {
        "title": "การถ่ายรูปคู่บัตรประชาชน",
        "desc": " <ul>\n        <li>  • บัตรต้องไม่บังใบหน้าส่วนใดส่วนหนึ่ง </li>\n      <br>\n      <li>  • ถอดแว่น ถอดแมสก่อนถ่าย </li><br>\n      <li>  • รูปต้องชัดเจนให้เห็นตัวอักษรบนบัตร</li><br>\n    </ul>",
        "footerBtn": "ถัดไป"
      },
      "scan": {
        "title": "กรุณาถ่ายรูปคู่ท่านและบัตรประชาชน ให้เห็นตัวอักษร และข้อมูลบนหน้าบัตรชัดเจน",
        "preview": {
          "title": "ยืนยันรูปคู่บัตรประชาชน",
          "footerPrimaryBtn": "ยืนยัน",
          "footerSecondaryBtn": "ถ่ายใหม่อีกครั้ง"
        }
      }
    },
    "reservePrivilege": {
      "checking": {
        "title": "ระบบกำลังทำการ ตรวจสอบสิทธิ์ของท่าน",
        "subtitle": "ทางระบบจะทำการจะส่งแจ้งเตือนไปยัง <br> ลูกค้าเมื่อได้ทำการเช็คสิทธิ์เรียบร้อย"
      },
      "confirmed": {
        "title": "สิทธิของท่านได้รับการ ยืนยันแล้ว!",
        "subtitle": "ระบบกำลังนำทางท่านไปพบคุณหมอ <br> ท่านสามารถสอบถามและปรึกษา อาการ ต่างๆ ของท่านได้เลย"
      },
      "rejected": {
        "title": "ขออภัย!สิทธิของท่านไม่ตรงตาม เงื่อนไขที่กำหนด",
        "subtitle": "กรุณาตรวจสอบสิทธิของท่านอีกครั้ง"
      }
    },
    "queuing": {
      "queueConfirmation": {
        "desc": "สามารถสอบถามและปรึกษา <br> อาการต่างๆของท่านได้เลย",
        "btnNavigateToMeet": "กดเข้าห้องเพื่อปรึกษาแพทย์"
      },
      "waitingMeetDoctor": {
        "title": "กรุณารอสักครู่ <br> คุณหมอกำลังเข้ามาให้<br> บริการท่านในอีกสักครู่ค่ะ",
        "subtitle": "ระบบกำลังนำทางท่านไปสู่การพบคุณหมอ <br> ท่านสามารถสอบถามและปรึกษาอาการ<br> ต่างๆ ของท่านได้เลย"
      }
    },
    "navigateBack": {
      "navigateBackNhso": {
        "title": "ขอบคุณสำหรับ\nการใช้บริการกับเรา",
        "desc": "ท่านสามารถกลับมาใช้สิทธิ์สปสช.\nผ่านหน้าหลักตามรูป",
        "btn": "กลับสู่หน้าหลัก"
      }
    },
    "welcomeNhso": {
      "welcomeNhso": {
        "title": "ยินดีต้อนรับสู่การใช้\nบริการสิทธิ์สปสช",
        "desc": "ระบบกำลังตรวจสอบสถานะ\nของท่านรุณารอสักครู่",
        "btn": "กลับสู่หน้าหลัก"
      }
    }
  }
};
